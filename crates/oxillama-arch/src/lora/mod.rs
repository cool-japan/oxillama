...
const LORA_A_SUFFIX: &str = ".lora_a";
const LORA_B_SUFFIX: &str = ".lora_b";
...
impl LoadedLora {
    ...
    pub fn from_gguf(model: &GgufModel) -> ArchResult<Self> {
        // --- Extract rank --------------------------------------------------
        // Try the canonical key first, then the alternative used by some tools.
        let rank = model
            .file
            .metadata
            .get("lora.r")
            ...
            .map(|v| v as usize)
            .unwrap_or(8); // sensible default when key is absent
        ...
        let alpha = model
            ...
            .unwrap_or(rank as f32); // default: alpha == rank → scale == 1.0
        ...
        for name in &tensor_names {
            ...
            // Dequantize A → [rank, in_features] f32
            let a_info = model
                .file
                .tensors
                .get(name)
            ...
            // Infer actual rank and in_features from tensor shape.
            // GGUF stores dimensions in column-major order: dims[0]=cols, dims[1]=rows
            // so for A of shape [in_features × rank] (C row-major):
            //   dims[0] = in_features, dims[1] = rank
            let (rank_actual, in_features) = shape_to_rank_in(a_info, rank, a_f32.len());

            // Dequantize B → [out_features, rank] f32
            let b_info = model
                .file
                .tensors
                .get(&b_name)
    ...
    /// # Example
    /// ```ignore
    /// if let Some(adapter) = loaded_lora.get("blk.0.attn_q.weight") {
    ///     layer.attn_q.set_lora(adapter);
    /// }
    /// ```
...
///
/// GGUF stores tensor dimensions in a non-obvious order (fastest-changing
/// dimension first, which is column-major / Fortran order).  For an A matrix
/// of mathematical shape `[rank × in_features]`:
///
/// * 2-D tensor: `dims = [in_features, rank]`  (GGUF col-major)
/// * 1-D tensor: assume `dims[0] = total_elements`, derive from `total / hint_rank`
/// * 0-D or unexpected: fall back to hint values
fn shape_to_rank_in(info: &TensorInfo, hint_rank: usize, n_elements: usize) -> (usize, usize) {
    match info.dimensions.as_slice() {
        ...
        _ => {
            // Unexpected shape: use hint rank and derive in_features from total
            let r = hint_rank.max(1);
            (r, n_elements / r)
        }
    }
...
///
/// Handles F32 and F16 as fast paths, then falls back to the block-based
/// kernel for any quantized type.
pub(crate) fn dequant_tensor_to_f32(
    ...
    // Fast path: F32 — direct little-endian copy.
    ...
    // Fast path: F16 — convert via the `half` crate.
    ...
    // Generic path: block-based dequantization.
...
mod tests {
    ...
    /// `LoadedLora` struct can be instantiated directly (compilation test).
    #[test]
    ...
    /// `shape_to_rank_in` correctly parses 2-D GGUF tensor shape.
    #[test]
    fn test_shape_to_rank_in_2d() {
        // GGUF 2-D: dims = [in_features=64, rank=8]
        let info = TensorInfo {
    ...
    /// `shape_to_rank_in` falls back correctly for 1-D tensors.
    #[test]
    fn test_shape_to_rank_in_1d() {
        let info = TensorInfo {
            name: "test.lora_a".into(),
            n_dims: 1,
            dimensions: vec![128], // total elements
            tensor_type: GgufTensorType::F32,
            offset: 0,
        };
        // hint_rank=8 → in_features = 128/8 = 16
        let (r, in_f) = shape_to_rank_in(&info, 8, 128);
        assert_eq!(r, 8);
        assert_eq!(in_f, 16);
    }

    /// get() returns None for a tensor not present in the adapter map.
    #[test]
    fn test_get_missing() {
        let lora = LoadedLora {
            adapters: HashMap::new(),
    ...
    /// A manually-constructed adapter is retrievable via `get()`.
    #[test]
    fn test_get_present() {
        let adapter =
    ...
    // ── Integration tests: round-trip through a synthetic LoRA GGUF ─────────
